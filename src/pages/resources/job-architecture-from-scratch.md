...
**Job architecture is live in Cadence today**: you can import your architecture, maintain **versioned role definitions**, and **assign roles** to people so that every employee maps to a definition at a level. Because definitions are versioned in the product, the governance model above stops being a filing discipline and becomes the default — revise in your semi-annual window, and the history stays retrievable. **Level context** then travels to where decisions happen: a manager preparing a growth conversation can work against the person's actual role expectations rather than a half-remembered PDF. [9-box calibration](https://cadencehr.ai/product) is Coming Q3 and not part of current purchase access.
